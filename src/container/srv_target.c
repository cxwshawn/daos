...
{
	return container_of(llink, struct ds_cont_child, scc_list);
}
	       ...
	       struct daos_llink **link)
{
	...
	uuid_copy(cont->scc_uuid, key);
	cont->scc_dtx_resync_time = time(NULL);

	rc = vos_cont_open(pool->spc_hdl, key, &cont->scc_hdl);
	if (rc != 0) {
		D_FREE(cont);
		return rc;
	}
...
cont_child_free_ref(struct daos_llink *llink)
{
	...
	D_DEBUG(DF_DSMS, DF_CONT": freeing\n", DP_CONT(NULL, cont->scc_uuid));
	vos_cont_close(cont->scc_hdl);
	D_FREE(cont);
}
		    ...
		    struct daos_llink *llink)
{
	...
	return uuid_compare(key, cont->scc_uuid) == 0;
}
...
{
	daos_lru_ref_release(cache, &cont->scc_list);
}
...
cont_close_one_rec(struct cont_tgt_close_rec *rec)
{
	...
	D_DEBUG(DF_DSMS, DF_CONT": closing: hdl="DF_UUID" hce="DF_U64"\n",
		DP_CONT(hdl->sch_pool->spc_uuid, hdl->sch_cont->scc_uuid),
...
cont_epoch_discard_one(void *vin)
{
	...
	rc = vos_discard(hdl->sch_cont->scc_hdl, &epr);
	if (rc > 0)	/* Aborted */
		rc = -DER_CANCELED;
