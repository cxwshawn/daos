...
dtx_handler(crt_rpc_t *rpc)
{
	...
	switch (opc) {
	case DTX_COMMIT:
		rc = vos_dtx_commit(cont->scc_hdl, din->di_dtx_array.ca_arrays,
				    din->di_dtx_array.ca_count);
		break;
	case DTX_ABORT:
		rc = vos_dtx_abort(cont->scc_hdl, din->di_dtx_array.ca_arrays,
	...
	case DTX_CHECK:
		...
		if (din->di_dtx_array.ca_count != 1)
			 ...
			 */
			rc = vos_dtx_check_committable(cont->scc_hdl, NULL,
